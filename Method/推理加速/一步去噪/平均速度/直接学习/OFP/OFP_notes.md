...
>
> 对于单个样本对 $(\epsilon,a)$ ，Flow Matching 构造的是直线路径，监督目标 $a-\epsilon$ 是这条直线的恒定斜率，因此既可以看作当前位置的局部速度，也可以看作任意区间的平均速度。但面对整个训练集时，不同样本会产生大量相互交叉的直线路径。对于某个位置 $(t,z_t)$ ，如果存在多条路径经过，模型会收到多个不同斜率的监督信号；为了最小化误差，模型最终学习这些斜率的条件均值。经过充分训练后，模型学到的是在整个 $(t,z_t)$ 空间中的连续速度场，即每个位置对应一个方向箭头。推理时，模型根据当前位置 $(t,z_t)$ 查询速度场。对于单个样本，斜率与对应直线路径强绑定，因此既表示局部速度，也表示整段区间的平均速度；但在多样本情况下，模型学习的是所有经过当前位置样本路径斜率的条件均值，该速度不再对应某一条具体路径，只描述当前点附近的局部运动方向。因此模型无法直接预测整段位移，只能沿速度场逐步积分，并在每一步重新查询当前位置的速度，经过多步迭代完成从噪声到数据的转换。由于不同位置对应的条件均值速度可能不同，最终积分得到的轨迹通常是一条曲线。简单来说：对于普通 Flow Matching 而言，模型学习的是斜率（单样本）或斜率均值（多样本），斜率本身表示的就是局部速度，只不过在单样本情况下因为速度场是直线，斜率也可以表示为平均速度，但是多样本情况下速度不再对应某一条具体路径，自然就不能表示为平均速度了。
>
