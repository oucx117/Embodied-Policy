...
| 大类                             | 建模范式                           | 核心问题                                                     |
| -------------------------------- | ---------------------------------- | ------------------------------------------------------------ |
| **VA** (Vision-Action)           | $p(a_{t+1:t+k}| o_t)$              | 不依赖语言，根据视觉观测直接生成动作                         |
| **VLA** (Vision-Language-Action) | $p(a_{t+1:t+k}|o_t,l)$             | 利用 VLM 的语义知识，根据语言指令泛化到新任务、新物体、新场景 |
| **WAM** (World-Action Model)     | $p(o_{t+1:t+k},a_{t+1:t+k}|o_t,l)$ | 预测未来状态（通常以视频方式表现）和物理动作                 |
