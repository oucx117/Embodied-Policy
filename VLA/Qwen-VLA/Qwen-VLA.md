...
> Qwen-VLA 与 `π0` 的一个结构差异是：`π0` 更像是在单一 Transformer 中设置 VLM expert 和 action expert 两套权重；Qwen-VLA 则更明确地将 Qwen3.5 VLM 作为认知主干，把 DiT action decoder 作为解码器式动作专家接在其上。解耦式架构设计让 Action Expert 专注细粒度动作生成，适配具身动作分布的多模态与高频动力学特性，同时保留 VLM 主干预训练能力。
